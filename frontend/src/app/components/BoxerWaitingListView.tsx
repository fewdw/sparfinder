...
import {BOXER_WAITING_LIST_VIEW_EVENTS, BOXER_LEAVE_WAITING_LIST_EVENT } from '../utils/apiConfig';
...
const BoxerWaitingListView = () => {
  ...
  const fetchWaitingEvents = async () => {
    ...
    try {
      ...
      setError(`Error fetching waiting events: ${error.message}`);
    } finally {
      setLoading(false);
    }
  };

  const handleLeaveWaitingEvent = async (eventUuid) => {
    const JWT = Cookies.get('jwt');
    try {
      ...
      if (response.ok) {
        setWaitingEvents(prevEvents => prevEvents.filter(event => event.uuid !== eventUuid));
      } else {
  ...
  return (
    <div>
        <MenuBar />
      <h2 className="text-lg font-semibold my-4 text-center">Your Waiting List Events</h2>
      <div className="max-w-4xl mx-auto">
        {waitingEvents.length > 0 ? (
              ...
              {waitingEvents.map(event => (
                <tr key={event.uuid}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                    <button
                      onClick={() => handleLeaveWaitingEvent(event.uuid)}
          ...
          <p className="text-center text-sm text-gray-500">No events found on the waiting list.</p>
        )}
      </div>
    </div>
  );
